...
def huffman_encode(arr, prefix, save_dir='./', verbose=False):
    """
    ...
    if verbose:
        print("dump")
        # arr = np.array([10, 10, 10, 10, 11,12,12,12,12,12])
        print (arr)
    ...
    if verbose:
        print ("write data:", datasize, data_encoding)
...
def huffman_encode_model(model, directory='encodings/'):
    ...
    original_total = 0
    compressed_total = 0
    print(f"{'Layer':<35} | {'original':>10} {'compressed':>10} {'improvement':>11} {'percent':>7}")
    print('-'*70)
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            shape = weight.shape
            # shape_str = "_".join(map(str,list(shape)))
            # print(shape_str)
            weightview = weight.contiguous().view(weight.shape[0], -1)
            shapeview = weightview.shape

            form = 'csr' if shapeview[0] < shapeview[1] else 'csc'
            mat = csr_matrix(weightview) if shapeview[0] < shapeview[1] else csc_matrix(weightview)
            # print(mat.data.shape, mat.indices.shape, mat.indptr, calc_index_diff(mat.indptr), reconstruct_indptr(calc_index_diff(mat.indptr)))
            ...
            t2, d2 = huffman_encode(calc_index_diff(mat.indptr), name+f'_{form}_indptr', directory)#, verbose=True)
            # t0, d0 = huffman_encode(mat.data, name+f'_{form}_{shape_str}_data', directory)
            # t1, d1 = huffman_encode(mat.indices, name+f'_{form}_{shape_str}_indices', directory)
            # t2, d2 = huffman_encode(calc_index_diff(mat.indptr), name+f'_{form}_{shape_str}_indptr', directory)

            # Print statistics
            original = mat.data.nbytes + mat.indices.nbytes + mat.indptr.nbytes
            compressed = t0 + t1 + t2 + d0 + d1 + d2

            print(f"{name:<35} | {original:10} {compressed:10} {original / compressed:>10.2f}x {100 * compressed / original:>6.2f}%")
        else: # bias
            # Note that we do not huffman encode bias
            # print("dump", directory+'/'+name, "param", param)
    ...
    print('-'*70)
    # print(original_total ,compressed_total, original_total/compressed_total)
...
def huffman_decode(directory, prefix, dtype, verbose=False):
    """
    ...
    # Read the codebook
    # print(f'huffman_decode {directory}/{prefix}')
    ...
    if verbose:
        print("dump")
        print([x for x in data_encoding])
...
def huffman_decode_model(model, directory='encodings/'):
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            form = 'csr' if shapeview[0] < shapeview[1] else 'csc'
            matrix = csr_matrix if shapeview[0] < shapeview[1] else csc_matrix
            ...
            # print("reconstruct", indptr)
            # exit()

            # Construct matrix
            # print(data.shape, indices.shape, indptr.shape)
            mat = matrix((data, indices, indptr), shapeview)

            # Insert to model
            param.data = torch.from_numpy(mat.toarray()).contiguous().view(shape).to(dev)
        else:
    ...
    for name, param in ms.items():
        if "running_mean" in name or "running_var" in name or "num_batches_tracked" in name:
            ...
            dev = param.device
            bias = np.load(directory+'/'+name, allow_pickle=True)            
            ms[name]= torch.from_numpy(bias).to(dev)
            # print(ms[name])
